    ...
    <div class="py-12">
        <div class="max-w-2xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="overflow-hidden shadow-sm sm:rounded-lg" style="background: var(--bg-card); border: 1px solid var(--border-main);">
                <div class="p-6">
                    <form action="{{ route('kelola-anggota.update', $anggota->id) }}" method="POST">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="mb-4">
                            ...
                            <select name="kelas" class="w-full rounded-md shadow-sm @error('kelas') border-red-500 @enderror" style="background: var(--bg-input); border: 1.5px solid var(--border-input); color: var(--text-primary);" required>
                                ...
                                <option value="X RPL 1" {{ old('kelas', $anggota->kelas) == 'X RPL 1' ? 'selected' : '' }}>X RPL 1</option>
                                <option value="X RPL 2" {{ old('kelas', $anggota->kelas) == 'X RPL 2' ? 'selected' : '' }}>X RPL 2</option>
                                <option value="XI RPL 1" {{ old('kelas', $anggota->kelas) == 'XI RPL 1' ? 'selected' : '' }}>XI RPL 1</option>
                                <option value="XI RPL 2" {{ old('kelas', $anggota->kelas) == 'XI RPL 2' ? 'selected' : '' }}>XI RPL 2</option>
                                <option value="XII RPL 1" {{ old('kelas', $anggota->kelas) == 'XII RPL 1' ? 'selected' : '' }}>XII RPL 1</option>
                                <option value="XII RPL 2" {{ old('kelas', $anggota->kelas) == 'XII RPL 2' ? 'selected' : '' }}>XII RPL 2</option>
                                <option value="X TKJ 1" {{ old('kelas', $anggota->kelas) == 'X TKJ 1' ? 'selected' : '' }}>X TKJ 1</option>
                                <option value="XI TKJ 1" {{ old('kelas', $anggota->kelas) == 'XI TKJ 1' ? 'selected' : '' }}>XI TKJ 1</option>
                                <option value="XII TKJ 1" {{ old('kelas', $anggota->kelas) == 'XII TKJ 1' ? 'selected' : '' }}>XII TKJ 1</option>
                            </select>
                            @error('kelas')
                            <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                            @enderror
                        ...
                        <!-- Password -->
                        <div class="mb-6">
                            <label class="block text-sm font-medium mb-2" style="color: var(--text-secondary);">Password Baru</label>
                            <input type="password" name="password" 
                                   class="w-full rounded-md shadow-sm @error('password') border-red-500 @enderror"
                                   style="background: var(--bg-input); border: 1.5px solid var(--border-input); color: var(--text-primary);"
                                   placeholder="Kosongkan jika tidak ingin mengubah password">
                            <p class="text-sm mt-1" style="color: var(--text-muted);">Kosongkan jika tidak ingin mengubah password</p>
                            @error('password')
                            <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                            @enderror
                        </div>
